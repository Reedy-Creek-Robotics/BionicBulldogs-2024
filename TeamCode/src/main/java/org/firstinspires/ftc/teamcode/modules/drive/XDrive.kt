...
@Config
class XDrive(config: XDriveConfig)
{
	...
	{
		if(localizer == null)
			throw RuntimeException("localizer cannot be null when calling driveFR");
		localizer!!.update();
	...
	{
		f =
			(forward * cos(heading.toDouble()) - strafe * sin(heading.toDouble())).toFloat();
		r =
			(forward * sin(heading.toDouble()) + strafe * cos(heading.toDouble())).toFloat();
		drive(f, r, rotate);
	}
	...
	fun drive(forward: Float, strafe: Float, rotate: Float)
	{
		...
		{
			//max motor power is bigger than max power so scale it back so the fastest motor is running at max power
	...
	fun debugTelemetry(t: Telemetry)
	{
		...
		t.addData(
			"total (current)",
			flEx.getCurrent(CurrentUnit.MILLIAMPS) +
					frEx.getCurrent(CurrentUnit.MILLIAMPS) +
					blEx.getCurrent(CurrentUnit.MILLIAMPS) +
